 ...
 *
 * <p>
 * 实现 {@link TypeMapper} 接口，用于 RowDescription 消息中的列类型 OID 字段。
 */
public class PgTypeMapper implements TypeMapper {
    ...
    public static int jdbcToPgOid(int jdbcType, String typeName) {
        ...
        if (typeName != null) {
            ...
            if (upper.contains("JSONB"))
                return PgOid.JSONB;
            if (upper.contains("JSON"))
                return PgOid.JSON;
            if (upper.contains("UUID"))
                return PgOid.UUID;
            if (upper.contains("TIMESTAMP") && upper.contains("TIME ZONE"))
     ...
     *
     * <p>
     * 定长类型返回具体字节数（1/2/4/8/16），变长类型统一返回 -1。
     */
