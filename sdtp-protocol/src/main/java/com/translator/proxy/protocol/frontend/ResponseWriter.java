 ...
 *
 * <p>
 * 不同协议（MySQL / PostgreSQL）对 OK、ERR、ColumnDef、EOF、TextRow 的线协议格式不同，由各协议模块实现此接口。
 */
public interface ResponseWriter {
     ...
     *            警告数
     * @param info
     *            附加信息（如 "Rows matched: 1 Changed: 1"）
     */
    void writeOk(
            byte sequenceNumber,
     ...
     *
     * <p>
     * 不同协议的消息序列不同（MySQL: ColumnCount + ColumnDef + EOF + Rows + EOF； PG: RowDescription + Rows + CommandComplete +
     * ReadyForQuery），由各实现自行决定。
     *
     * @param ctx
     *            Netty 上下文
     * @param rs
