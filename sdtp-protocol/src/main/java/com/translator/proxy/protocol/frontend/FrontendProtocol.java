 ...
 *
 * <p>
 * 每个协议实现模块（如 sdtp-protocol 的 MySQL、sdtp-protocol-pg 的 PostgreSQL） 通过 Java {@link java.util.ServiceLoader}
 * 注册其实现，sdtp-server 在启动时加载。
 *
 * <p>
 * 该接口定义了协议所需的全部工厂方法，包括编解码器、握手认证、 命令处理、响应写入、类型映射和系统目录查询。
 */
public interface FrontendProtocol {
     ...
     *            认证配置
     * @param executor
     *            业务线程池（用于 pipeline 中 handler 的线程绑定）
     * @return 握手处理器实例
     */
     ...
     *
     * <p>
     * 通用启动类（sdtp-server）在加载前端协议后调用一次，将全局配置中的 连接级参数（如最大包大小）下发给协议自身，由各协议决定如何应用， 从而避免通用层耦合任何具体协议的专属逻辑。默认空实现。
     *
     * <p>
     * 注意：参数使用基本类型而非 {@code ProxyConfig}，是为了保持 sdtp-protocol 模块对 sdtp-server 的无反向依赖。
     *
     * @param maxPacketSize
     *            最大允许包大小（字节），用于解码器 OOM 防护及协议相关默认值
     */
